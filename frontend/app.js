...
// ── CONFIG ─────────────────────────────────────────────────────────────────────
const API_URL = 'https://saludplus-api.onrender.com'; // ← cambiar si es necesario
...
// ── BOOT ───────────────────────────────────────────────────────────────────────
// FIX: todos los listeners se registran UNA SOLA VEZ aquí.
// showLogin() / showPanel() solo manipulan visibilidad CSS — nunca llaman init*.
document.addEventListener('DOMContentLoaded', () => {
  lucide.createIcons();

  // Registrar todos los listeners una única vez
  initLoginForm();
  initLogout();
  initTabs();
  initServicioForm();
...
// ── SCREENS ────────────────────────────────────────────────────────────────────
// FIX: estas funciones SOLO cambian visibilidad. No llaman ningún init*.
function showLogin() {
...
function initLoginForm() {
  ...
  form?.addEventListener('submit', async e => {
    ...
    try {
      ...
      setLoading(
        btn, false,
        '<i data-lucide="log-in" class="w-4 h-4"></i><span>Ingresar</span>'
      );
      lucide.createIcons();
    }
  });
...
async function loadServicios() {
  ...
  try {
    ...
    // Poblar serviciosMap para el selector de FAQs
    serviciosMap = {};
...
function initServicioForm() {
  ...
  // Auto-generar slug desde el nombre (solo al crear)
  ...
  form?.addEventListener('submit', async e => {
    ...
    try {
      ...
      setLoading(
        btn, false,
        '<i data-lucide="save" class="w-4 h-4"></i><span id="sv-submit-label">Crear servicio</span>'
      );
      lucide.createIcons();
    }
  });
...
function editServicio(id) {
  adminFetch('/servicios')
    ...
    .then(data => {
      ...
      document.getElementById('tab-servicios')
...
async function loadFaqs() {
  ...
  try {
    ...
    container.innerHTML = data.map(f => {
      const especNombre =
        f.servicios?.nombre ||
        (f.servicio_id ? serviciosMap[f.servicio_id] : null) ||
        '';
      return `
        <div class="list-row fade-in">
          <div class="list-row-main">
...
function initFaqForm() {
  ...
  form?.addEventListener('submit', async e => {
    ...
    try {
      ...
      setLoading(
        btn, false,
        '<i data-lucide="save" class="w-4 h-4"></i><span id="fq-submit-label">Crear FAQ</span>'
      );
      lucide.createIcons();
    }
  });
...
function editFaq(id) {
  adminFetch('/faqs')
    ...
    .then(data => {
      ...
      document.getElementById('tab-faqs')
        ?.querySelector('.panel-card')
        ?.scrollIntoView({ behavior: 'smooth', block: 'start' });
    });
}
...
// Llamado desde onclick inline en el HTML
...
function setLoading(btn, loading, restoreHtml) {
  ...
  btn.innerHTML = loading ? `<span class="opacity-70">${restoreHtml}</span>` : restoreHtml;
}
